...
- **Vault additions** → verify the link first, and pick the tool by where you're running:
  ...
  - **In a cloud routine:** verification only works if the environment's network egress policy allows `youtube.com`. If it doesn't, **both `curl` AND `WebFetch` fail with a 403/EGRESS_BLOCKED** — they route through the same gateway, so `WebFetch` is *not* a workaround (this was wrongly documented Jul–Aug 2026). Diagnose with `curl -sS "$HTTPS_PROXY/__agentproxy/status"`: a `connect_rejected … 403` for `youtube.com` means the policy blocks it. That is an infra failure, not "no highlight exists" — hold the event (don't prune) and report `⚠️ VERIFICATION BLOCKED`. The fix is to allow `youtube.com` in the environment's egress policy (Claude Code web → environment settings), which only Brad can do; `WebSearch` still works (it runs server-side) but can't do the oEmbed pass/fail check.
  - Either way, stamp `verified:` with today's date.

## Notes & Quirks

- **The cloud environment's egress policy blocks youtube.com (a 403 policy denial on all non-allowlisted hosts — youtube, espn, google; only Anthropic services, package registries, and GitHub are reachable).** From Jul 23–27 2026 the daily intake verified links with `curl`, got an error every morning, and — following "skip if unverifiable" — silently added **zero** vault entries for 5 days. Seven moments (TdF stages 19–21, CrossFit Games finals, Starship F13) were nearly lost. We *thought* `WebFetch` was the fix, but from ~Aug 6 2026 stage-20 verification failed via `WebFetch` too (same gateway, same 403): the routine correctly held the event and escalated for days rather than pruning it — proof the "hold + ⚠️ VERIFICATION BLOCKED" reflex works even when the documented workaround doesn't. **The real fix is allowing `youtube.com` in the environment's egress policy** (Brad, in Claude Code web settings); a `build.py` warning fires when a locked event with a `result:` is within 3 days of being pruned with no matching vault `event_id`. **Never let "the verifier broke" collapse into "nothing qualified"** — those are different outcomes and only one of them is silent.
